...
function refreshChannelDropdown(node, app) {
    ...
    if (validChannels.length === 0) {
        ...
        if (!node._isLoading && channelWidget.value !== "Unavailable") {
        ...
        if (!channelWidget.value || channelWidget.value === "Unavailable" || channelWidget.value === "Default") {
    ...
    nodeType.prototype.onNodeCreated = function () {
        ...
        node.networkSyncCheck = function () {
            ...
            if (!record && targetChannel !== "Available" && targetChannel !== "Unavailable" && targetChannel !== "Default") {
                ...
                const active = scanActiveBroadcasters(app);
                channelWidget.value = active.length > 0 ? "Available" : "Unavailable";
                ...
                // Check individual track integrity
                if (node.outputs) {
                    node.outputs.forEach(out => {
